...
select segment, count(product_code) as unique_product_counts from dim_product
group by segment 
...
group by segment )
    select segment, product_count_2020, product_count_2021, product_count_2021 - product_count_2020 as difference from info_;
    
-- Get the products that have the highest and lowest manufacturing costs.
 select fact_manufacturing_cost.product_code, product, manufacturing_cost from fact_manufacturing_cost 
 left join 
...
select dim_customer.customer_code, customer, avg(pre_invoice_discount_pct) as average_discount_percentage
from dim_customer
right join 
fact_pre_invoice_deductions on (dim_customer.customer_code = fact_pre_invoice_deductions.customer_code)
...
select month(date) as month_, fact_sales_monthly.fiscal_year,
...
group by month(date), fact_sales_monthly.fiscal_year
...
select division, dim_product.product_code, product,
    sum(sold_quantity) as total_sold_quantity,
    dense_rank() over(order by sum(sold_quantity) asc) as rnk
from dim_product
join 
fact_sales_monthly on (dim_product.product_code = fact_sales_monthly.product_code)
